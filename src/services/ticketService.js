...
export const riteTicket = async (ticketData) => {
    // ticketData debe contener al menos el identificador (folio) y los campos a actualizar
    try {
        const response = await fetch(`${BASE_URL}/TicketRate`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(ticketData),
        });
